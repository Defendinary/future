...
class ElasticsearchPlugin(IPlugin):
    # __type__ = "database"
    ...
    def __init__(self, host, username, password):
        self.es = AsyncElasticsearch(host, basic_auth=(username, password))
        self.host = host
        self._username = username
        self._password = password
        #self.auth = ES_AUTH

    async def create_index(self, index_name, mapping=None, settings=None):
        ...
        try:
            ...
            result = await self.es.indices.create(index=index_name, body=payload)
    ...
    async def delete_index(self, index_name):
        """Delete an index"""
        try:
            result = await self.es.indices.delete(index=index_name)
    ...
    async def get_index_info(self, index_name=None):
        """Get information about an index or all indices"""
        try:
            if index_name:
                result = await self.es.indices.get(index=index_name)
            else:
                result = await self.es.cat.indices(format="json")
            return result
        except Exception as e:
            return {"error": str(e)}

    async def create_alias(self, index_name, alias_name):
        """Create an alias for an index"""
        try:
            result = await self.es.indices.put_alias(index=index_name, name=alias_name)
            return {"status": f"Alias {alias_name} created for index {index_name}", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def delete_alias(self, index_name, alias_name):
        """Delete an alias"""
        try:
            result = await self.es.indices.delete_alias(index=index_name, name=alias_name)
            return {"status": f"Alias {alias_name} removed from index {index_name}", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def index_document(self, index_name, document, document_id=None):
        """Index a single document"""
        try:
            if document_id:
                result = await self.es.index(index=index_name, id=document_id, body=document, refresh=True)
            else:
                result = await self.es.index(index=index_name, body=document, refresh=True)
    ...
    async def bulk_index(self, index_name, documents, bulk_options=None):
        ...
        try:
            ...
            result = await async_bulk(self.es, index=index_name, actions=documents)
            return result
        except Exception as e:
            return {"error": str(e)}

    async def bulk_actions(self, actions, raise_on_error=False):
        """Bulk index/update/delete using elasticsearch.helpers.bulk action dicts."""
        try:
            success, errors = await async_bulk(self.es, actions, raise_on_error=raise_on_error)
            error_count = len(errors) if errors else 0
            return success, error_count
        except Exception:
            return 0, 1 if raise_on_error else 0

    async def scroll_document_ids(self, index_name, query=None, page_size=1000, scroll_ttl="2m"):
        ...
        while hits:
            response = await self.es.scroll(scroll_id=scroll_id, scroll=scroll_ttl)
            scroll_id = response.get("_scroll_id")
            hits = response.get("hits", {}).get("hits", [])
            known.update(hit["_id"] for hit in hits)
    ...
    async def search_documents(self, index_name, query, search_options=None):
        ...
        try:
            ...
            result = await self.es.search(index=index_name, body=query)
            return result
        except Exception as e:
            return {"error": str(e)}
    ...
    async def search(self, index_name, **kwargs):
        ...
        try:
            return await self.es.search(index=index_name, **kwargs)
        except Exception as e:
            return {"error": str(e)}

    async def get_document(self, index_name, document_id):
        """Get a specific document by ID"""
        try:
            result = await self.es.get(index=index_name, id=document_id)
            return result
        except Exception as e:
            return {"error": str(e)}

    async def update_document(self, index_name, document_id, update_data):
        """Update a document"""
        try:
            result = await self.es.update(index=index_name, id=document_id, body={"doc": update_data})
            return {"status": "Document updated", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def delete_document(self, index_name, document_id):
        """Delete a document"""
        try:
            result = await self.es.delete(index=index_name, id=document_id)
    ...
    async def get_index_settings(self, index_name):
        """Get index settings"""
        try:
            result = await self.es.indices.get_settings(index=index_name)
            return result
        except Exception as e:
            return {"error": str(e)}

    async def update_index_settings(self, index_name, settings):
        """Update index settings"""
        try:
            result = await self.es.indices.put_settings(index=index_name, body=settings)
            return {"status": "Settings updated", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def get_index_mapping(self, index_name):
        """Get index mapping"""
        try:
            result = await self.es.indices.get_mapping(index=index_name)
            return result
        except Exception as e:
            return {"error": str(e)}

    async def update_index_mapping(self, index_name, mapping):
        """Update index mapping"""
        try:
            result = await self.es.indices.put_mapping(index=index_name, body=mapping)
            return {"status": "Mapping updated", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def reindex(self, source_index, dest_index, query=None):
        ...
        try:
            ...
            result = await self.es.reindex(body=body)
    ...
    async def refresh_index(self, index_name):
        ...
        try:
            result = await self.es.indices.refresh(index=index_name)
            return {"status": f"Index {index_name} refreshed", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def flush_index(self, index_name):
        """Flush an index to disk"""
        try:
            result = await self.es.indices.flush(index=index_name)
            return {"status": f"Index {index_name} flushed", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def rename_index(self, old_index_name, new_index_name, delete_old=True):
        ...
        try:
            ...
            if not await self.index_exists(old_index_name):
                return {"error": f"Source index {old_index_name} does not exist"}
            
            # Step 2: Check if new index already exists
            if await self.index_exists(new_index_name):
                return {"error": f"Target index {new_index_name} already exists"}
            
            # Step 3: Get the mapping and settings from old index
            old_mapping = await self.es.indices.get_mapping(index=old_index_name)
            old_settings = await self.es.indices.get_settings(index=old_index_name)
            ...
            await self.es.indices.create(index=new_index_name, body=new_index_body)
            ...
            reindex_result = await self.es.reindex(body=reindex_body)
            
            # Step 6: Delete old index if requested
            if delete_old:
                await self.es.indices.delete(index=old_index_name)
                return {
    ...
    async def copy_index(self, source_index_name, target_index_name):
        """Copy an index to a new name (keeps original)"""
        return await self.rename_index(source_index_name, target_index_name, delete_old=False)

    async def get_updatable_settings(self, index_name):
        """Get a list of index settings that can be updated"""
        try:
            # Get current settings
            current_settings = await self.es.indices.get_settings(index=index_name)
    ...
    async def update_index_metadata(self, index_name, metadata_updates):
        ...
        try:
            ...
            # Update the settings
            result = await self.es.indices.put_settings(index=index_name, body=metadata_updates)
            return {
    ...
    async def get_index_stats(self, index_name):
    ...
    async def force_merge_index(self, index_name, max_num_segments=1):
        """Force merge index segments for optimization"""
        try:
            result = await self.es.indices.forcemerge(index=index_name, max_num_segments=max_num_segments)
            return {"status": f"Index {index_name} force merged", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def close_index(self, index_name):
        """Close an index (makes it read-only and frees up memory)"""
        try:
            result = await self.es.indices.close(index=index_name)
            return {"status": f"Index {index_name} closed", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def open_index(self, index_name):
        """Open a closed index"""
        try:
            result = await self.es.indices.open(index=index_name)
            return {"status": f"Index {index_name} opened", "result": result}
        except Exception as e:
            return {"error": str(e)}

    async def create_api_key(self, index_name, alias="logs", elastic_user=None, elastic_pass=None, privileges=None, role_name="writer", expiration=None, metadata=None):
        ...
        try:
            ...
            try:
                ...
                await self.es.indices.put_alias(index=index_name, name=alias, body={"is_write_index": True})
            except Exception:
                # Fallback to simple alias if cluster/version does not support body param
                await self.es.indices.put_alias(index=index_name, name=alias)
    ...
    async def bulk_insert(self, index_name, docs, bulk_options=None):
        ...
        try:
            ...
            if bulk_options:
                result = await self.es.bulk(body=bulk_body, **bulk_options)  # compare to: es.bulk_actions(actions, raise_on_error=False)
            else:
                result = await self.es.bulk(body=bulk_body)
